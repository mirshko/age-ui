...
<button
  ...
  class={`relative min-w-0 flex-1 overflow-hidden bg-white py-4 px-4 text-center text-sm font-medium hover:bg-gray-50 focus:z-10 ${
    $selectedTab === tab ? "text-gray-900" : "text-gray-500 hover:text-gray-700"
  }} first:rounded-l-lg last:rounded-r-lg`}
  aria-current={$selectedTab === tab ? "page" : "false"}
>
  <slot />
  <span
    aria-hidden="true"
